    ...
    <div className="relative min-h-screen bg-background">
      ...
      <main className="relative z-10 mx-auto max-w-6xl px-6 pb-24">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {members.map((m) => {
            ...
            return (
              <article
                key={m.name}
                className={`surface-plate group flex items-center gap-4 rounded-xl border border-border p-5 transition-all duration-300 hover:-translate-y-0.5 ${accent?.ring ?? "hover:border-gold/50"}`}
              >
                <div
        ...
        <div className="mt-14 rounded-xl border border-ichor/25 surface-plate px-8 py-10 text-center">
          ...
          <a
            href={DISCORD_INVITE}
            className="mt-6 inline-block rounded-md bg-primary px-7 py-3 font-mono text-sm font-semibold tracking-widest text-primary-foreground uppercase transition-all hover:shadow-[0_0_30px_oklch(0.48_0.19_25/50%)]"
          >
            Enter the Discord
          </a>
        </div>
